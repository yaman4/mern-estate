...
export default function CreateListing() {
  ...
  const handleImageSubmit = (e) => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 6) {
      ...
      setImageUploadError('Only 5 images can be uploaded');
      setUploading(false);
    }
  };
    ...
    <main className='p-3 max-w-4xl mx-auto'>
      ...
      <form className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className='flex flex-col flex-1 gap-4'>
          <p className='font-semibold'>Images (max 5)</p>
          <div className='flex gap-4'>
            <input onChange={(e)=>setFiles(e.target.files)} className='p-3 border border-gray-300 rounded' type='file' id='images' accept='image/*' multiple
             />
            <button onClick={handleImageSubmit} type='button' className='p-3 text-green-700 border border-green-700 rounded 
